...
int socketentry()
{
	...
	for(i=0;i<sockemax;i++)
	{
		...
		dest_addr[0].sin_port=htons(_PORT_);
#ifdef WIN32	
		InetPton(AF_INET, (PCWSTR)L_ADDR_, &(dest_addr[i].sin_addr));
#else
		inet_pton(AF_INET, _ADDR_, &(dest_addr[i].sin_addr));
#endif
	}
